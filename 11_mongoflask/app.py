...
def meteorites_with_class(class_name: str) -> list:
    ...
    return render_template("index.html", Selection = "Lists all the meteorites classified with class_name given.", class_name = list(meteoriteLandings.find({"recclass": class_name})))

@app.route('/meteoritesfound')
def meteorites_found() -> list:
    ...
    """
    return render_template("index.html", Selection = "List of all the meteorites that were found (that means someone found it rather than someone saw it falling)", meteoritesfound= list(meteoriteLandings.find({"fall": "Found"})))

@app.route('/meteoritesmax')
def meteorite_with_class_and_max_mass(class_name: str, max_mass: float):
    ...
    :return:
    """
    max_mass = request.args['max_mass']
    class_name = request.args['class_name']
    return render_template("index.html", Selection = "Lists all the meteorites classified with class_name given limited by given maximum mass.", meteoritesmax = list(meteoriteLandings.find({"recclass": class_name, "mass": {"$lt": max_mass}})))

@app.route('/meteoritesdate')
def meteorite_with_class_and_found_before_date(class_name: str, date: datetime.datetime):
    ...
    class_name = request.args['class_name']
    return render_template("index.html", Selection = "List of all meteorites of given class that fell before the given time", meteoritesdate = result)

@app.route('/meteoritesfellnear')
def meteorite_fell_near(location: str):
    ...
    return render_template("index.html", Selection = "Finds all the meteorite that fell near your area given the location. Error of margin is 25 degree distance sqrt(a**2 + b**2)", meteoritesfellnear = output)
